...
# csrf_token = s.get(URL).cookies['csrftoken']
        ...
        v = 'http://portal.lnct.ac.in/Accsoft2/parents/ParentDesk.aspx'
        ...
        if LoginState:
            ...
            raw = str(soup.find('table', id='ctl00_ContentPlaceHolder1_Gridview1'))
            rawList = raw.split('<tr>')
            # print(rawList[2])
            for x in range(2, len(rawList)):
                rawTemp = rawList[x].split('<td>')
                sno = (rawTemp[1].split('<'))[0]
                date = (rawTemp[2].split('<'))[0]
                period = (rawTemp[3].split('<'))[0]
                subject = (rawTemp[4].split('<'))[0]
                attendState = (rawTemp[5].split('<'))[0]
                # print(sno,date,period,subject,attendState)
                temp = {
                    'sno.': sno,
                    'date': date,
                    'period': period,
                    'subject': subject,
                    'Attend Status': attendState
                }
                result.append(temp)
            return result
        else:
