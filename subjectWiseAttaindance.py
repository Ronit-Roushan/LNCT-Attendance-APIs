...
# csrf_token = s.get(URL).cookies['csrftoken']
        ...
        if LoginState:
            soup = BeautifulSoup(s.get(URL+'Accsoft2/parents/subwiseattn.aspx').text, 'html.parser')
            raw = str(soup.find('table', id='ctl00_ContentPlaceHolder1_grd'))
            rawList = raw.split('<tr>')
            # print(rawList[2])
            for x in range(1, len(rawList)):
                rawTemp = rawList[x].split('<td>')
                subject = (rawTemp[1].split('<'))[0]
                code = (rawTemp[2].split('<'))[0]
                total_lectures = (rawTemp[3].split('<'))[0]
                attindance = (rawTemp[4].split('<'))[0]
